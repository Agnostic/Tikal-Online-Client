...
now.getStats = function(){
	if(!init && typeof(now.player.attr) == "undefined")
		return false;
	if(typeof(now.player.attr.hp != "undefined")){
		$("#hp").css({"width": now.player.attr.hp+"%"});
		$("#hp_txt").html(now.player.attr.hp);
	}
}
